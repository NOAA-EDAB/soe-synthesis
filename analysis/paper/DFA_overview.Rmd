...
model_plot <- ggplot() +
  ...
  geom_label_repel(
    data = mod_dat %>% filter(!is.na(best_model)), aes(x = m, y = delta_AICc), label = "Best Model",
...
trend_dat <- best_mod %>% 
  ...
  # group_by(EPU, Season) %>%
  # mutate(trends = map(dat, tidy, type = "states")) %>% 
  unnest(trends) %>%
